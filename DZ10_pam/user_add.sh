...
echo -e "day        ALL=(ALL)       NOPASSWD: ALL" >> /etc/sudoers.d/day #Даем sudo пользоателю day (Без проверки пароля)
# Добавлем строку с путем скрипту проверки гуппы юзеров 
sed -i "2i auth  required  pam_exec.so /usr/local/bin/test_login.sh"  /etc/pam.d/sshd
